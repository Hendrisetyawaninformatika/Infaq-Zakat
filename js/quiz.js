...
async function loadQuizzes() {
    try {
        const snapshot = await db.collection('quiz').where('status', '==', 'active').orderBy('createdAt', 'desc').get();
        allQuizzes = []; snapshot.forEach(doc => { allQuizzes.push({ id: doc.id, ...doc.data() }); });
        renderQuizList();
        loadLeaderboard();
    } catch (error) { console.error('Error loading quizzes:', error); showToast('Gagal memuat quiz', 'error'); }
}
...
function renderQuizList() {
    ...
    if (allQuizzes.length === 0) {
        container.innerHTML = `<div class="empty-state"><i class="fas fa-mosque"></i><p>Belum ada quiz yang tersedia</p></div>`;
        return;
    }
...
async function startQuiz(quizId) {
    ...
    const existing = await db.collection('quizResults').where('userId', '==', currentUser.uid).where('quizId', '==', quizId).get();
    if (!existing.empty) {
        const result = existing.docs[0].data();
        if (result.score >= 70) { showToast('Anda sudah lulus quiz ini dengan nilai ' + result.score + '%', 'info'); return; }
    }
    const quiz = allQuizzes.find(q => q.id === quizId);
    if (!quiz) { showToast('Quiz tidak ditemukan', 'error'); return; }
    const questionsSnapshot = await db.collection('quizQuestions').where('quizId', '==', quizId).get();
    const questions = [];
    questionsSnapshot.forEach(doc => { questions.push({ id: doc.id, ...doc.data() }); });
    if (questions.length < 5) { showToast('Quiz ini belum memiliki cukup soal', 'error'); return; }
...
async function submitQuiz() {
    ...
    try {
        await db.collection('quizResults').add({
            userId: currentUser.uid, userName: currentUserData?.name || 'User',
            quizId: currentQuiz.id, quizTitle: currentQuiz.title, quizCategory: currentQuiz.category,
            score: score, correct: correct, total: currentQuestions.length, timeTaken: timeTaken,
            answers: results, createdAt: firebase.firestore.FieldValue.serverTimestamp()
        });
        showResults(score, correct, timeTaken, results);
    } catch (error) { console.error('Error saving quiz result:', error); showToast('Gagal menyimpan hasil quiz', 'error'); }
}
...
function showResults(score, correct, timeTaken, results) {
    ...
    document.getElementById('resultMessage').textContent = passed ? 'Selamat! Anda berhasil menyelesaikan quiz dengan nilai yang memuaskan.' : 'Terus belajar dan jangan menyerah! Coba lagi ya.';
...
function startTimer() {
    ...
    timer = setInterval(() => {
        ...
        if (timeLeft <= 60) { timerDisplay.style.color = '#F44336'; }
        if (timeLeft <= 0) { clearInterval(timer); showToast('Waktu habis! Quiz akan disubmit otomatis.', 'warning'); submitQuiz(); }
    }, 1000);
}
    ...
    results.forEach((result, index) => {
        html += `
            <div class="review-item ${result.isCorrect ? 'correct' : 'wrong'}">
                <div class="review-question"><span class="review-number">${index + 1}.</span><span>${result.question}</span></div>
                <div class="review-answers"><span>Jawaban Anda: <strong>${result.userAnswer}</strong></span><span>Jawaban Benar: <strong>${result.correctAnswer}</strong></span></div>
...
function retakeQuiz() {
    ...
    currentQuiz = null; currentQuestions = []; currentQuestionIndex = 0; userAnswers = []; clearInterval(timer);
}

async function generateCertificate() {
    if (!window.quizPassed) { showToast('Anda harus lulus quiz untuk mendapatkan sertifikat', 'warning'); return; }
    try {
        showToast('Membuat sertifikat...', 'info');
        const certData = {
            name: currentUserData?.name || 'User', quizTitle: currentQuiz?.title || 'Quiz Islami',
            score: window.quizScore || 0, date: formatDate(new Date()),
            certificateId: 'CERT-' + generateId(8).toUpperCase()
        };
        showCertificateModal(certData);
    } catch (error) { console.error('Error generating certificate:', error); showToast('Gagal membuat sertifikat', 'error'); }
}
...
function showCertificateModal(data) {
    ...
    modal.innerHTML = `
        <div class="modal-content" style="max-width:500px;">
            <div class="modal-header"><h3><i class="fas fa-certificate" style="color:var(--gold);"></i> Sertifikat Quiz</h3>
            <button class="modal-close" onclick="document.getElementById('certificateModal').remove()">&times;</button></div>
            <div class="modal-body" style="text-align:center;">
                <div style="border:3px solid var(--gold); border-radius:12px; padding:30px; background:linear-gradient(135deg, #fff8e1, #fff3e0);">
                    ...
                    <div style="border-top:1px dashed var(--gray-300); padding-top:12px; font-size:12px; color:var(--gray-500);">ID: ${data.certificateId}</div>
                </div>
                <button class="btn btn-primary" onclick="printCertificate()" style="margin-top:16px;"><i class="fas fa-print"></i> Cetak Sertifikat</button>
            </div>
        </div>
    `;
...
function printCertificate() {
    ...
    printWindow.document.write(`
        ...
        <style>body{font-family:'Poppins',sans-serif;display:flex;justify-content:center;align-items:center;min-height:100vh;margin:0;background:#f5f5f5;}
        ...
        .certificate i{font-size:60px;color:#FFD700;}.certificate h2{margin:12px 0 4px;}.certificate .name{font-size:24px;color:#2E7D32;}
        .certificate .title{color:#F9A825;}.certificate .score{font-size:28px;color:#2E7D32;}
        .certificate .id{border-top:1px dashed #ccc;padding-top:12px;font-size:12px;color:#999;}</style>
        </head><body>
        <div class="certificate"><i class="fas fa-award"></i><h2>Sertifikat Kelulusan</h2>
        <p style="color:#999;">Diberikan kepada:</p><div class="name">${content.querySelector('h3').textContent}</div>
        <p>Atas keberhasilan menyelesaikan quiz</p><h4 class="title">"${content.querySelector('h4').textContent}"</h4>
        <div style="display:flex; justify-content:center; gap:40px; margin:16px 0;">
        ...
        <div class="id">ID: ${content.querySelector('.id')?.textContent?.trim() || ''}</div></div>
        </body></html>
    `);
...
async function loadLeaderboard() {
    ...
    if (!container) return;
    try {
        const snapshot = await db.collection('quizResults').orderBy('score', 'desc').limit(10).get();
        if (snapshot.empty) { container.innerHTML = '<p class="text-muted" style="text-align:center;">Belum ada peserta</p>'; return; }
        let html = '<ol style="padding-left:20px;">'; let rank = 1;
        snapshot.forEach(doc => {
            const result = doc.data();
            const medal = rank === 1 ? '🥇' : rank === 2 ? '🥈' : rank === 3 ? '🥉' : `${rank}.`;
            html += `<li style="display:flex; justify-content:space-between; padding:8px 0; border-bottom:1px solid var(--gray-200);">
                <span>${medal} ${result.userName || 'User'}</span><span style="font-weight:600; color:var(--primary);">${result.score}%</span></li>`;
            rank++;
        });
        html += '</ol>';
        container.innerHTML = html;
